...
static class WriteConfPass : public Pass {
  void usage() const;
  void run(DesignState &ds, const std::vector<std::string> &args) const;
public:
  WriteConfPass() : Pass("write_conf") {}
} write_conf_pass;

void
WriteConfPass::usage() const
{
  std::cout
    << "  " << name() << " <txt-file>\n"
    << "\n"
    << "    Write configuration based on current place and route to <txt-file>.\n"
    << "    `-' for standard output.\n";
}

void
WriteConfPass::run(DesignState &ds, const std::vector<std::string> &args) const
{
